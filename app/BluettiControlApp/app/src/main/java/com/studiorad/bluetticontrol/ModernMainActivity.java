...
public class ModernMainActivity extends MainActivity {
    ...
    android.graphics.drawable.Drawable rgbFrame(int fill,int radius){
        ...
        inner.setCornerRadius(dp(radius-1));
    ...
    @Override TextView tv(String t,int sp,int c){TextView v=new TextView(this);v.setText(t);v.setTextSize(sp);v.setTextColor(c==Color.DKGRAY?muted:c);v.setShadowLayer(sp>=16?10:4,0,0,c);v.setPadding(dp(14),dp(6),dp(14),dp(6));if(sp>=18)v.setTypeface(Typeface.DEFAULT,Typeface.BOLD);v.setSingleLine(false);return v;}
    @Override Button btn(String t){Button b=new Button(this);b.setText(t);b.setAllCaps(false);b.setTextColor(Color.WHITE);b.setTextSize(14);b.setTypeface(Typeface.DEFAULT,Typeface.BOLD);b.setShadowLayer(10,0,0,Color.WHITE);b.setMinHeight(dp(62));b.setElevation(dp(14));b.setBackground(rgbButton());b.setPadding(dp(10),dp(10),dp(10),dp(10));return b;}
    @Override TextView metric(String title){TextView v=tv(title+": --",16,navy);v.setBackground(rgbFrame(cardBg2,28));v.setElevation(dp(12));v.setGravity(Gravity.CENTER_VERTICAL);v.setPadding(dp(16),dp(16),dp(16),dp(16));return v;}
    @Override LinearLayout card(){LinearLayout c=new LinearLayout(this);c.setOrientation(LinearLayout.VERTICAL);c.setPadding(dp(20),dp(18),dp(20),dp(18));c.setBackground(rgbFrame(cardBg,28));c.setElevation(dp(12));LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,-2);lp.setMargins(dp(14),dp(10),dp(14),dp(10));c.setLayoutParams(lp);return c;}
    ...
    @Override void buildUi(){
        ...
        LinearLayout topbar=new LinearLayout(this);topbar.setGravity(Gravity.CENTER_VERTICAL);topbar.setPadding(dp(10),dp(4),dp(10),dp(4));topbar.addView(tv("☰",28,blue2),new LinearLayout.LayoutParams(0,-2,1));TextView logo=tv("BLUETTI CONTROL",26,navy);logo.setGravity(Gravity.CENTER);topbar.addView(logo,new LinearLayout.LayoutParams(0,-2,3));TextView menu=tv("⚙",24,violet2);menu.setGravity(Gravity.RIGHT);topbar.addView(menu,new LinearLayout.LayoutParams(0,-2,1));root.addView(topbar);
    ...
    TextView stat(String icon,String label,String value,int color){TextView v=tv(icon+"  "+label+"\n"+value,18,color);v.setGravity(Gravity.CENTER_VERTICAL);v.setBackground(rgbFrame(cardBg2,28));v.setElevation(dp(12));v.setPadding(dp(16),dp(16),dp(16),dp(16));if(label.equals("Bateria"))statBattery=v;else if(label.equals("Entrada"))statInput=v;else if(label.equals("Saída"))statOutput=v;else statDc=v;return v;}
    Button control(String text,int color){Button b=btn(text);b.setTextColor(color);b.setTextSize(17);b.setMinHeight(dp(82));b.setShadowLayer(12,0,0,color);b.setBackground(rgbButton());return b;}
