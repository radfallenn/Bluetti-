...
public class BluettiPersistentService extends Service {
    ...
    static final long CHECK_INTERVAL_MS = 10L * 60L * 1000L;
    static final long QUICK_RETRY_MS = 30L * 1000L;
    ...
    @Override public void onCreate() {
        super.onCreate();
        createChannel();
        startForeground(NOTIFICATION_ID, buildNotification("Bluetti Control ativo", "Monitoramento em segundo plano"));
    ...
    private void checkConnection() {
        ...
        if (ready && gatt != null && writeChar != null) {
            readStatus();
            updateNotification("Conectado • checado há 0 min");
            return;
        }
        if (connecting) return;
        if (gatt != null && !ready && now - lastSeen > QUICK_RETRY_MS) safeClose();
        connectLast();
    }
    ...
    private void connectLast() {
        ...
        if (mac == null || mac.length() < 17) {
            updateNotification("Sem último dispositivo salvo");
            return;
        }
        try {
            ...
            updateNotification("Reconectando em segundo plano");
        } catch (Exception e) {
            connecting = false;
            updateNotification("Erro ao reconectar");
        }
    }
    ...
    private final BluetoothGattCallback callback = new BluetoothGattCallback() {
        @Override public void onConnectionStateChange(BluetoothGatt g, int status, int state) {
            if (state == BluetoothProfile.STATE_CONNECTED) {
                ...
                handler.postDelayed(() -> { try { if (gatt != null) gatt.discoverServices(); } catch (Exception ignored) {} }, 600);
                updateNotification("Conectado, carregando serviços");
            } else if (state == BluetoothProfile.STATE_DISCONNECTED) {
                ready = false; writeChar = null; connecting = false;
                try { g.close(); } catch (Exception ignored) {}
                gatt = null;
                updateNotification("Desconectado • nova tentativa em 10 min");
                handler.postDelayed(() -> { if (!ready) checkConnection(); }, QUICK_RETRY_MS);
            }
        }
        @Override public void onMtuChanged(BluetoothGatt g, int mtu, int status) {
            try { if (gatt != null) gatt.discoverServices(); } catch (Exception ignored) {}
        }
        @Override public void onServicesDiscovered(BluetoothGatt g, int status) {
            ...
            else updateNotification("Serviços Bluetti não encontrados");
        }
        @Override public void onDescriptorWrite(BluetoothGatt g, BluetoothGattDescriptor d, int status) {
            ready = status == 0 && writeChar != null;
            lastSeen = System.currentTimeMillis();
            if (ready) { readStatus(); updateNotification("Conectado em segundo plano"); }
            else updateNotification("Falha ao ativar notify");
        }
    ...
    private void enableNotify(BluetoothGatt g, BluetoothGattCharacteristic c) {
        try {
            ...
            else { ready = writeChar != null; if (ready) readStatus(); }
    ...
    private String statusText() {
        if (lastBattery >= 0) return "Bateria " + lastBattery + "% • Consumo " + lastLoad + " W • checa a cada 10 min";
        return ready ? "Conectado em segundo plano • checa a cada 10 min" : "Monitoramento persistente • checa a cada 10 min";
    }

    private void safeClose() {
        try { if (gatt != null) { gatt.disconnect(); gatt.close(); } } catch (Exception ignored) {}
        gatt = null; writeChar = null; ready = false; connecting = false;
    }
    ...
    private Notification buildNotification(String title, String text) {
        Intent openIntent = new Intent(this, ModernMainActivity.class);
