...
public class CustomDashboardActivity extends LastDeviceActivity {
    ...
    @Override void buildUi() {
        ...
        LinearLayout debugCard = card(); debugCard.addView(tv("BLE Debug AC180P", 20, navy)); bleDebug = tv("Modo seguro: aguardando parser...", 12, muted); debugCard.addView(bleDebug); Button clearDebug = btn("Limpar debug"); clearDebug.setOnClickListener(v -> { rxCount = 0; bleDebug.setText("Debug limpo."); }); debugCard.addView(clearDebug); root.addView(debugCard);
    ...
    @Override void parseRead(byte[] pkt) {
        try {
            if (pkt == null || pkt.length < 5) { showSafeDebug("Pacote curto ou nulo"); return; }
            int bc = pkt[2] & 255; int words = bc / 2; if (words < 0) return;
            int[] w = new int[words];
            for (int i=0; i<words && (4+i*2)<pkt.length; i++) w[i] = ((pkt[3+i*2]&255)<<8) | (pkt[4+i*2]&255);
            final int fWords = words; final int fLen = pkt.length; final int[] fw = w;
            runOnUiThread(() -> {
                showSafeDebug("Parse OK: words=" + fWords + " bytes=" + fLen + " HEX=" + bytesToHex(pkt));
        ...
        } catch (Exception e) { showSafeDebug("Erro parse: " + e.getMessage()); }
    }

    private void showSafeDebug(String s) { try { if (bleDebug != null) runOnUiThread(() -> { rxCount++; String t = "#" + rxCount + " " + s + "\n\n" + bleDebug.getText(); bleDebug.setText(t.length() > 2000 ? t.substring(0, 2000) : t); }); } catch(Exception ignored){} }
}
